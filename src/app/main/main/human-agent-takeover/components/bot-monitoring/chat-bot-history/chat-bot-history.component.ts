...
export class ChatBotHistoryComponent implements OnInit {
  ...
  knowledgeBase: any = true;
  suggestion: any;
  activeIntervalId: any
  constructor(
    private _chatVisibilityS: ChatVisibilityTakeoverService,
  ...
  checkActiveItems() {
    this.activeIntervalId = setInterval(() => {
      ...
      const activeChatsString = localStorage.getItem('activeChatsAgentTakeover');
  ...
  getChatDetails(activeChat: any) {
    ...
    const data = { session_id: activeChat.session_id, last_message: activeChat.last_message, sentiment: activeChat.sentiment };
    if (this.chats.length == 1) {
      this._chatVisibilityS.notifythirdActiveHistory({ active: false, session_id: activeChat.session_id });
      alert('The maximum number of visible screens is limited to one.');
      return [];
    }
    ...
    return this._botS.ChatHistory(formData).pipe(
      exhaustMap((res: any) => {
        ...
        if (res.detail.length > 0) {
          ...
          res.detail['sentiment'] = data.sentiment;

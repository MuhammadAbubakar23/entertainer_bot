...
import { ChatVisibilityTakeoverService } from 'src/app/services/chat-visibility-takeover.service';
import { bot_id, workspace_id, chat_path, bot_name } from 'src/main';
declare var toggleNavPanel: any;
@Component({
  selector: 'app-expanded-human-agent-takeover',
  templateUrl: './expanded-human-agent-takeover.component.html',
  styleUrls: ['./expanded-human-agent-takeover.component.scss']
})
export class ExpandedHumanAgentTakeoverComponent {
  ...
  bot_id = bot_id;
  constructor(private chatVisibilityService: ChatVisibilityTakeoverService, public InitialsService: InitialsService,
      ...
      if (obj) {
        ...
        if (index != -1) {
          ...
          if (openChats.length > 0) {
            localStorage.setItem('activeChatsAgentTakeover', JSON.stringify(openChats));
          }
          else {

            localStorage.removeItem('activeChatsAgentTakeover');
          }
        }
      }
    })
  ...
  getChatBotHistory() {
    ...
    this._botService.chatBotHistoryForAgentTakeover(formData).subscribe((res: any) => {
